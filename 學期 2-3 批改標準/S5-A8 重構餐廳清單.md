...
參考課程平台([原文連結](https://lighthouse.alphacamp.co/courses/42/assignments/1039))。
...
1. 延續之前的【餐廳專案】規格，進行程式碼重構
   1. 參考 RESTful 設計，運用 HTTP 動詞來讓路由帶有語義
   2. 使用 express.Router 獨立路由器，並規劃路由模組
2. 首頁 index 頁面需與更新的 wireframe 一致
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>程式邏輯與 Coding Style</td>
      <td>思考程式的可維護性：當下拉選單的類別變多時，你目前的程式邏輯還能使用嗎？ (如果是用 hard code 方式就一定不可能)</td>
    </tr>
      <tr>
      <td>視覺與使用者動線</td>
      <td>(略)</td>
